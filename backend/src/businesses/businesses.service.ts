...
export class BusinessesService {
  ...
  constructor(@InjectMapper() private readonly mapper: Mapper) {
    ...
    const crateId = data => data.displayed_what.toLowerCase().split(' ').join('-');

    Promise.all(ids.map(id => axios.get(`https://storage.googleapis.com/coding-session-rest-api/${id}`)))
      .then(response => this.businesses = response.map(f => new Business({ ...f.data, id: crateId(f.data) })));
  }
  ...
  getBusinesses(searchText?: string): any {
    ...
    const findBusinessByText = business => [business.displayed_what, business.displayed_where]
